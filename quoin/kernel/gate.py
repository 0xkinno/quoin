...
class TwoPhaseCommitGate:
    ...
    def __init__(self, authority_reader: Callable[[], AuthoritySnapshot]):
    ...
    def commit(
        ...
        # 2. Phase 2 Read-After-Write: Re-read current active authority from memory
        current_snapshot = self.authority_reader()

        # 3. Generation Match Check (The Core Fence)
        if receipt.policy_generation != current_snapshot.generation:
            self.invalidated_receipts.add(receipt_id)
            return CommitResult(
                committed=False,
                ...
                error=(
                    ...
                    f"but active visible authority advanced to G{current_snapshot.generation}. Aborting commit."
                ),
                audit_trail={
                    "receipt_generation": receipt.policy_generation,
                    "active_generation": current_snapshot.generation,
                    "aborted_at": now.isoformat(),
                },
            )

        # 4. Policy Hash Check
        if receipt.policy_hash != current_snapshot.retrieved_record_hash:
            self.invalidated_receipts.add(receipt_id)
            return CommitResult(
                committed=False,
                status="ABORTED_POLICY_HASH_MISMATCH",
                error="Active policy digest differs from authority receipt policy digest.",
                audit_trail={"receipt_hash": receipt.policy_hash, "active_hash": current_snapshot.retrieved_record_hash},
            )

        # 5. Request Mutation / Tamper Check
        ...
        # 6. Proposal Mutation / Tamper Check
        ...
        # 8. All Predicates Satisfied: Issue Cryptographic Execution Permit
        permit_id = CanonicalHasher.compute_permit_id(receipt_id, current_effect_hash)
        signature = CanonicalHasher.sign_permit(permit_id, receipt_id, current_effect_hash, current_snapshot.generation)

        permit = ExecutionPermit(
            permit_id=permit_id,
            receipt_id=receipt_id,
            request_id=receipt.request_id,
            policy_generation=current_snapshot.generation,
